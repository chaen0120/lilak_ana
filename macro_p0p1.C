...
void macro_p0p1(TString type = "14Oap")
{
    ...
    TFile *fin0 = new TFile(Form("ana/p0/ana_%s.root", type.Data()));
    TTree *tree0 = (TTree *)fin0->Get("event");
    TFile *fin1 = new TFile(Form("ana/p1/ana_%s.root", type.Data()));
    TTree *tree1 = (TTree *)fin1->Get("event");

    TClonesArray *EnderArray0 = nullptr;
    TClonesArray *HeaderArray0 = nullptr;
    tree0->SetBranchAddress("EventEnder", &EnderArray0);
    tree0->SetBranchAddress("EventHeader", &HeaderArray0);
    TClonesArray *EnderArray1 = nullptr;
    TClonesArray *HeaderArray1 = nullptr;
    tree1->SetBranchAddress("EventEnder", &EnderArray1);
    tree1->SetBranchAddress("EventHeader", &HeaderArray1);
    auto nEvts = tree0->GetEntries();
    ...
    for (int iEvt = 0; iEvt < nEvts; iEvt++)
    {
        ...
        tree0->GetEntry(iEvt);
        auto header0 = (TTEventHeader *)HeaderArray0->At(0);
        auto ender0 = (TTEventEnder *)EnderArray0->At(0);
        tree1->GetEntry(iEvt);
        auto header1 = (TTEventHeader *)HeaderArray1->At(0);
        auto ender1 = (TTEventEnder *)EnderArray1->At(0);

        auto eventNo = header0->GetEventNumber();
        auto firedDet = header0->GetFiredDet();
        auto firedStrip = header0->GetFiredStrip();
        if (!CutBadDetectors(firedDet, firedStrip)) continue;

        auto SiE = ender0->GetSiE();
        auto CsIE = ender0->GetCsIE();
        auto SiPos = ender0->GetSiHit();
        int iDet = -1;
        for (int i = 0; i < 40; i++)
        ...
        auto runNo = ender0->GetRun();
        auto splitNo = ender0->GetSplit();
        auto LT = liveTime[runNo];
        if (LT < 0.5) continue;

        auto SiEcm = ender0->GettEcm();
        auto SiAlab = ender0->GettAlab();
        auto Sivert = TVector3(0, 0, ender0->GettZ());
        auto SiLen = (SiPos - Sivert).Mag();
        auto TPCEcm0 = ender0->GetpEcm();
        if (reac == 'a') TPCEcm0 = ender0->GetaEcm();
        auto TPCEcm1 = ender1->GetpEcm();
        if (reac == 'a') TPCEcm1 = ender1->GetaEcm();
        auto TPCAlab = ender0->GetAlab();
        auto TPCvert = ender0->GetVertex();
        auto TPCLen = (SiPos - TPCvert).Mag();

        auto dEdx = ender0->GetdEdx();
        auto Edet = ender0->GetEdet();
        if (dEdx>0 && Edet>0) his_dEdx[iDet]->Fill(Edet,dEdx);
        if (TPCEcm0 == -1 || TPCEcm1 == -1) continue;
        ...
        his_EdvsSZEcm[iDet]->Fill(ZtoE(ender1->GettZ()), Edet);
        ...
        double TPCEcm = (TPCEcm0 + TPCEcm1) / 2;
        ...
        auto errTh = CalculateThetaError(TPCAlab, SiPos, ender0->GetSiHitError(), TPCvert);
        auto errEcm = CalculateEcmError(Edet, TPCAlab, errEp, errTh);
        errEcm += abs(TPCEcm0 - TPCEcm1) / 2;
